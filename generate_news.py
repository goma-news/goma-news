...
rss_urls = [
    ...
    "https://www.investing.com/rss/news_11.rss",  # 상품 뉴스
    "https://www.investing.com/rss/news_95.rss",  # 경제지표
    "https://www.forexlive.com/feed/"  # ForexLive 실시간 뉴스
]
...
for rss_url in rss_urls:
    try:
        ...
        for item in items:
            ...
            link = item.link.text
            pub_date = item.pubDate.text if item.pubDate else now

            # 키워드 포함 여부 확인
            if any(k in title for k in keywords):
                summary_prompt = f"뉴스 제목: {title}\n요약: 이 뉴스 내용을 한국어로 해석해주고, 해외선물과 관련된 것인지 판단해서 요점을 한두 문장으로 요약해줘."
                try:
